...
public class AuthFrame extends JFrame {
    ...
    private void handleRegister() {
        ...
        if (username.isEmpty()) {
            setStatus(regStatusLabel, "⚠  Username cannot be empty.", ERROR_COLOR);
            return;
        }
        if (password.isEmpty()) {
            setStatus(regStatusLabel, "⚠  Password cannot be empty.", ERROR_COLOR);
            return;
        }
        if (!password.equals(confirm)) {
            setStatus(regStatusLabel, "⚠  Passwords do not match.", ERROR_COLOR);
            return;
        }
        if (password.length() < 6) {
            setStatus(regStatusLabel, "⚠  Password must be at least 6 characters.", ERROR_COLOR);
            return;
        }
    ...
    private void handleLogin() {
        ...
        if (username.isEmpty()) {
            setStatus(logStatusLabel, "⚠  Username cannot be empty.", ERROR_COLOR);
            return;
        }
        if (password.isEmpty()) {
            setStatus(logStatusLabel, "⚠  Password cannot be empty.", ERROR_COLOR);
            return;
        }
